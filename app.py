...
with st.spinner('⏳ Processing 500,000+ transactions...'):
    ...
    if df.empty:
        st.error("Data could not be loaded. Check your internet or local files.")
        st.stop()
...
# 1. Filter the main data by country FIRST
filtered_df = df[df['Country'] == selected_country]

# 2. Run AI Segmentation on ONLY the filtered data (The Fix!)
with st.spinner(f'🤖 Running AI Analysis for {selected_country}...'):
...
with col_right:
    st.write(f"### 🤖 AI Customer Segments ({selected_country})")
